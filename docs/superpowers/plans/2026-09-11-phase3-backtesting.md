...
def _metrics(equity: pd.Series, trade_pnls: list[float]) -> BacktestResult:
...
def run_funding_backtest(funding: pd.Series, threshold: float = 0.0001,
    ...
    2 * fee (two legs). Basis risk assumed zero (documented approximation).
    """
    positioned = funding > threshold
    flips = positioned.astype(int).diff().fillna(0.0).abs()
    cost = flips * 2.0 * fee
    ...
    return _metrics(equity, trade_pnls)
```
